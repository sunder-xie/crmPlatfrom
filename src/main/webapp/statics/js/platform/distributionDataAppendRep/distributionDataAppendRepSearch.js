			...
			.datagrid(
					{
						...
						title : '查询结果',
						url : appUrl
								+ '/distributionDataAppendRepAction!getDistributionDataAppendRepList.jspa',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
						...
						toolbar : [
								...
								"-",
								{
									...
									handler : function() {
										exportDistributionDataAppendRep();
									}
								},
								"-",
								{
...
// excel导出
function exportDistributionDataAppendRep() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionDataAppendRepAction!exportDistributionDataAppendRep.jspa';
	form.submit();
	$.messager.alert('Tips', '正在导出,请稍候!', 'warning');
}

function closeDtPlan() {
	$("#MaintDistributionDataAppendRep").window('close');
}

function initMaintDistributionDataAppendRep(title, url, WWidth, WHeight) {
	var url = appUrl + url;
	var $win = $("#MaintDistributionDataAppendRep")
			.window(
					{
						title : title,
						width : WWidth,
...
function updateDistributionDataAppendRep(ctId) {//
	var ctId = encodeURIComponent(ctId);
	var mark = encodeURIComponent(mark);
	initMaintDataAppend('目标数修改',
			'/goalAction!updateDataAppendPrepare.jspa?ctId=' + ctId + '&mark'
					+ mark, 490, 400);
}

function importDistributionDataAppend() {
						...
						buttons : [
								{
									...
									handler : function() {
										...
										if (/^.+\.(csv|CSV)$/.test(file)) {
											...
											var action = '';
											action = appUrl
													+ "/distributionDataAppendRep/distributionDataAppendRepAction!importDistributionDataAppendRepCsv.jspa";
											var form = document
...
function selectOrgTree() {
	initMaintDistributionDataAppendRep('选择组织',
			'/distributionDataAppendRepAction!orgTreePage.jspa', 400, 460);
}
...
function importMatterModel() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionDataAppendRep/distributionDataAppendRepAction!importDistributionDataAppendRepModel.jspa';
	form.target = "hideFrame";
	form.submit();
}

function deleteDistributionDataAppendRep() {
	...
	$("#ids").val(ids);
	var form = window.document.forms[0];
	form.action = "distributionDataAppendRepAction!deleteDistributionDataAppendRep.jspa";
	form.target = "hideFrame";
	form.submit();
...
function saverow(target) {
	...
	var form = window.document.forms[0];
	form.action = "distributionDataAppendRepAction!updateDistributionDataAppendRep.jspa";
	form.target = "hideFrame";
	form.submit();
}
...
function exportMould() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionDataAppendRep/distributionDataAppendRepAction!exportMouldCsv.jspa';
	form.target = "hideFrame";
	form.submit();
}
