			...
			.datagrid(
					{
						...
						title : '提交结果',
						url : appUrl
								+ '/kunnrBusinessContact/kunnrBusinessAction!getDealerAdjustDelJsonList.jspa?id='+$('#id').val(),
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
...
function importDealerAdjustment() {
	$.messager.alert('Tips', '流程正在处理或者已完成不能上传文件！');
	return;
...
function deleteChageRow() {
	$.messager.alert('Tips', '流程正在处理或者已完成不能删除文件');
	return;
...
function exportMould() {
	$.messager.alert('Tips', '流程正在处理中或者已完成不能下载文件');
	return;
	/*var form = window.document.forms[0];
	form.action = appUrl
