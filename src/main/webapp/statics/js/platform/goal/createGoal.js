...
function loadGrid1() {
	$('#wid').combogrid({
		...
		method : 'post',
		// multiple : true,
		url : appUrl + '/goal/goalAction!getMaterielViewList.jspa',
		columns : [ [ {
			field : 'ck',
			checkbox : true,
			hidden : true
...
function searcher(val) {
	...
	$('#id').combogrid({
		//getKunnerJsonList
		url : appUrl + '/goal/goalAction!getKunner.jspa?value=' + val
	});
...
function submit() {
	...
	var pa = $("#theMonth").combobox('isValid');
	...
	$.messager.confirm('Confirm', '是否确认提交?', function(r) {
		if (r) {
			...
			form.action = appUrl + "/goal/goalAction!createGoal.jspa";
