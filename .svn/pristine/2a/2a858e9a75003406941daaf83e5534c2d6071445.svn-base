package com.kintiger.platform.distributionDataAppendRep.action;
...
public class DistributionDataAppendRepAction extends BaseAction {

	private static final long serialVersionUID = -532423459213585547L;
	private static Log logger = LogFactory
			.getLog(DistributionDataAppendRepAction.class);
	...
	private IDistributionDataAppendRepService distributionDataAppendRepService;
	private List<DistributionDataAppendRep> distributionDataAppendRepLists;
	private String uploadFileFileName;
	private File uploadFile;
	private String xmlFilePath;
	private DistributionDataAppendRep dGoal;
	private IKunnrService kunnrService;

	public String distributionDataAppendRepSearch() {
		userId = this.getUser().getUserId();
		Borg org = orgServiceHessian.getOrgByUserId(userId);
		orgId = org.getOrgId().toString();
		return "distributionDataAppendRepSearch";
	}
	 ...
	 */
	@PermissionSearch
	@JsonResult(field = "distributionDataAppendRepLists", total = "size", include = {
			...
			"channel", "store_category", "resign_date","createDate" })
	public String getDistributionDataAppendRepList() {
		distributionDataAppendRepLists = new ArrayList<DistributionDataAppendRep>();
		DistributionDataAppendRep dGoal = new DistributionDataAppendRep();
		...
		size = distributionDataAppendRepService
				.getDistributionDataAppendRepCount(dGoal);
		if (size != 0) {
			distributionDataAppendRepLists = distributionDataAppendRepService
	...
	/*public String updateDistributionDataAppend() {
		...
		String[] m = { "A1","A2", "A3", "A4", "A5", "A6", "A7", "A8", "B1", "B8",
				"C1", "C6", "C7", "C8", "D9","D10","E1", "H7" };
		int[] b = { aOne,aTwo,  aThree, aFour, aFive, aSix, aSeven, aEight, bOne,
				bEight, cOne, cSix, cSeven, cEight, dNine,dTen,eOne, hSeven };
	...
	public String exportMouldCsv() {
		...
		String report_name = "主管业代分销数据导入模板";
		...
		try {
			...
			linedata.append(",");
			linedata.append("A2椰果六连杯-80g*6*6");
			linedata.append(",");
			linedata.append("A3椰果经典组合装-80g*30");
			linedata.append(",");
			...
			linedata.append(",");
			linedata.append("A8椰果三连杯-80g*3*10");
			linedata.append(",");
			linedata.append("B1红豆单杯-64g*30");
			linedata.append(",");
			linedata.append("B8红豆三连杯-64g*3*10");
			linedata.append(",");
			linedata.append("C1桂圆红枣单杯-65g*30");
			linedata.append(",");
			linedata.append("C6红豆+桂圆红枣家庭分享装-80g*16*6");
			linedata.append(",");
			linedata.append("C7红豆+桂圆红枣家庭分享装-80g*15*6");
			linedata.append(",");
			linedata.append("C8桂圆红枣三连杯装-65g*3*10");
			linedata.append(",");
			linedata.append("D9超值组合装");
			linedata.append(",");
			linedata.append("D10豪华礼盒装-6*15");
			linedata.append(",");
			linedata.append("E1豪华礼盒装-6*12");
			linedata.append(",");
		...
		try {
			...
			if (StringUtils.isNotEmpty(uploadFileFileName)) {
				...
				if ((end != null && (end.equals(".csv")))
					...
					String[] m = { "A1","A2", "A3", "A4", "A5", "A6", "A7", "A8",
							"B1", "B8", "C1", "C6", "C7", "C8", "D9","D10","E1", "H7" };
					DistributionDataAppend dGoal;
	...
	private void List2Excel(List<DistributionDataAppendRep> dbList) {
		OutputStream os = null;
		...
		try {
			...
			response.reset();
			// 设定输出文件头
			String fileName = "主管业代分销数据报表.xls";
			...
			wsheet.addCell(new Label(5, 0, "业务姓名", wcfFG));
			wsheet.addCell(new Label(6, 0, "岗位(主管/业代)", wcfFG));
			wsheet.addCell(new Label(7, 0, "直接上级姓名", wcfFG));
			wsheet.addCell(new Label(8, 0, "直接上级岗位", wcfFG));
			wsheet.addCell(new Label(9, 0, "渠道类别", wcfFG));
			wsheet.addCell(new Label(10, 0, "负责的门店类型", wcfFG));
			wsheet.addCell(new Label(11, 0, "分销日期", wcfFG));
			wsheet.addCell(new Label(12, 0, "A1椰果单杯", wcfFT));
			wsheet.addCell(new Label(12, 1, "80g*30", wcfFT));
			wsheet.addCell(new Label(13, 0, "A2椰果联杯装", wcfFT));
			wsheet.addCell(new Label(13, 1, "80g*6*6", wcfFT));
			...
			wsheet.addCell(new Label(18, 0, "A7椰果家庭分享装", wcfFT));
			wsheet.addCell(new Label(18, 1, "80g*15*6", wcfFT));
			wsheet.addCell(new Label(19, 0, "A8椰果联杯装", wcfFT));
			wsheet.addCell(new Label(19, 1, "80g*3*10", wcfFT));
			wsheet.addCell(new Label(20, 0, "B1红豆单杯", wcfFT));
			wsheet.addCell(new Label(20, 1, "64g*30", wcfFT));
			wsheet.addCell(new Label(21, 0, "B6红豆家庭分享装", wcfFT));
			wsheet.addCell(new Label(21, 1, "64g*16*6", wcfFT));
			wsheet.addCell(new Label(22, 0, "B8红豆联杯装", wcfFT));
			wsheet.addCell(new Label(22, 1, "80g*3*10", wcfFT));
			wsheet.addCell(new Label(23, 0, "C1桂圆红枣单杯", wcfFT));
			wsheet.addCell(new Label(23, 1, "65g*30", wcfFT));
			wsheet.addCell(new Label(24, 0, "C6桂圆红枣家庭分享装", wcfFT));
			wsheet.addCell(new Label(24, 1, "65g*16*6", wcfFT));
			wsheet.addCell(new Label(25, 0, "C7桂圆红枣家庭分享装", wcfFT));
			wsheet.addCell(new Label(25, 1, "65g*15*6", wcfFT));
			wsheet.addCell(new Label(26, 0, "C8桂圆红枣联杯装", wcfFT));
			wsheet.addCell(new Label(26, 1, "65g*3*10", wcfFT));
			wsheet.addCell(new Label(27, 0, "D9超值组合装", wcfFT));
			wsheet.addCell(new Label(28, 0, "D10豪华礼盒装", wcfFT));
			wsheet.addCell(new Label(28, 1, "15*6", wcfFT));
			wsheet.addCell(new Label(29, 0, "E1豪华礼盒装", wcfFT));
			wsheet.addCell(new Label(29, 1, "12*6", wcfFT));
			wsheet.addCell(new Label(30, 0, "H7功夫奶茶单瓶", wcfFT));
			wsheet.addCell(new Label(30, 1, "400ml*15", wcfFT));
			wsheet.addCell(new Label(31, 0, "录入人", wcfFG));
			wsheet.addCell(new Label(32, 0, "业务人员离职日期", wcfFG));
			for (int i = 1; i <= dbList.size(); i++) {
				wsheet.addCell(new Label(0, i+1,
						dbList.get(i - 1).getOrgRegion(), wcfF));
				wsheet.addCell(new Label(1, i+1, dbList.get(i - 1)
						.getOrgProvince(), wcfF));
				wsheet.addCell(new Label(2, i+1, dbList.get(i - 1).getOrgCity(),
						wcfF));
				wsheet.addCell(new Label(3, i+1, dbList.get(i - 1).getKunnrId(),
						wcfF));
				wsheet.addCell(new Label(4, i+1,dbList.get(i - 1).getKunnrName(), 
						wcfF));
				wsheet.addCell(new Label(5, i+1,
						dbList.get(i - 1).getFirstUser(), wcfF));
				wsheet.addCell(new Label(6, i+1, dbList.get(i - 1).getPosition(),
						wcfF));
				wsheet.addCell(new Label(7, i+1, dbList.get(i - 1)
						.getDire_super_name(), wcfF));
				wsheet.addCell(new Label(8, i+1, dbList.get(i - 1)
						.getDire_super_position(), wcfF));
				wsheet.addCell(new Label(9, i+1, dbList.get(i - 1).getChannel(),
						wcfF));
				wsheet.addCell(new Label(10, i+1, dbList.get(i - 1)
						.getStore_category(), wcfF));
				wsheet.addCell(new Label(11, i+1, dbList.get(i - 1)
						.getInputDate(), wcfF));
				wsheet.addCell(new Number(12, i+1, Double.valueOf(dbList.get(
						i - 1).getaOne()), wcfF));
				wsheet.addCell(new Number(13, i+1, Double.valueOf(dbList.get(
						i - 1).getaTwo()), wcfF));
				wsheet.addCell(new Number(14, i+1, Double.valueOf(dbList.get(
						i - 1).getaThree()), wcfF));
				wsheet.addCell(new Number(15, i+1, Double.valueOf(dbList.get(
				...
				wsheet.addCell(new Number(30, i+1, Double.valueOf(dbList.get(
						i - 1).gethSeven()), wcfF));
				wsheet.addCell(new Label(31, i+1, dbList.get(i - 1)
						.getInputName(), wcfF));
				wsheet.addCell(new Label(32, i+1, dbList.get(i - 1)
						.getResign_date(), wcfF));
			}*/
	...
	public String exportDistributionDataAppendRep() {
		OutputStream os = null;
		WritableWorkbook wbook = null;
		List<String> props = new ArrayList<String>();
		distributionDataAppendRepLists = new ArrayList<DistributionDataAppendRep>();
		DistributionDataAppendRep dbDataAppendRep = new DistributionDataAppendRep();
		...
		try {
			...
			distributionDataAppendRepLists = distributionDataAppendRepService
					.getDistributionDataAppendRepList(dbDataAppendRep);

			if (distributionDataAppendRepLists.size() == 0) {
				this.setFailMessage("Excel数据导出出错,请不要导出数据为空的列表");
			}
			List2Excel(distributionDataAppendRepLists);
			ServletActionContext.getRequest().getSession()
	...
	public static void setLogger(Log logger) {
		DistributionDataAppendRepAction.logger = logger;
	}
	...
	public IDistributionDataAppendRepService getDistributionDataAppendRepService() {
		return distributionDataAppendRepService;
	}

	public void setDistributionDataAppendRepService(
			IDistributionDataAppendRepService distributionDataAppendRepService) {
		this.distributionDataAppendRepService = distributionDataAppendRepService;
	}

	public void setDistributionDataAppendRepLists(
			List<DistributionDataAppendRep> distributionDataAppendRepLists) {
		this.distributionDataAppendRepLists = distributionDataAppendRepLists;
	}

	public List<DistributionDataAppendRep> getDistributionDataAppendRepLists() {
		return distributionDataAppendRepLists;
	}
	...
	public DistributionDataAppendRep getdGoal() {
		return dGoal;
	}

	public void setdGoal(DistributionDataAppendRep dGoal) {
		this.dGoal = dGoal;
	}
