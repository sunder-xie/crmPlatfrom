			...
			.datagrid(
					{
						iconCls : 'icon-list',
						title : '库存列表',		
						url : appUrl + '/stockReport/stockReportAction!searchStockReportListByKunnrSales.jspa',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
...
//							iconCls : 'icon-add',
//							handler : function() {
//								if($("#loginId").val()=="88647" || $("#loginId").val()=="90479"){
//									importCsv(); 
//								}
//							}
//						}, "-"
...
function addWeek() {
	if($("#state").val()=="1"){
		if($("#flag").val()=="kunnr_week" || $("#flag").val()=="both"){
			initMaintWindow('创建库存', '/stockReportAction!toCreateStockReportWeek.jspa?orgId='+$("#orgId").val()+'&&userType='+$("#userType").val(), 500, 500);
		}else{
...
function addMonth() {
	if($("#state").val()=="1"){
		if($("#flag").val()=="kunnr_month" || $("#flag").val()=="both"){
			initMaintWindow('创建库存', '/stockReportAction!toCreateStockReportMonth.jspa?orgId='+$("#orgId").val()+'&&userType='+$("#userType").val(), 500, 500);
		}else{
			$.messager.alert('提示', '不是提报时间！', '提示');
		}
	}else{
