...
$(document)
		.ready(
				function() {
					...
					// 客户科目组
					$('#ktokd').combobox({
						//url : appUrl + '/kunnrAction!getKtokd.jspa',//方法不存在，所以注释掉  by cg.jiang 201607
...
function submit() {
	...
	if($('#channelId').combobox('getValue')){
	    ...
	    array2json();
	    /**商业折扣页面已经注释而js未注释，by cg.jiang 201607
	    if($('#text0').val()==''&&$('#text01').val()==''&&$('#text02').val()==''){
			$.messager.alert('Tips', '商业折扣的SAP未填写!', 'warning');
			return;
		}
		if($('#text1').datebox('getValue')==''){
			$.messager.alert('Tips', '商业折扣的开始生效日期未填写!', 'warning');
			return;
		}
        if($('#text2').datebox('getValue')==''){
        	$.messager.alert('Tips', '商业折扣的有效截止日期未填写!', 'warning');
        	return;
		}*/
		/*if(acountCount>0){
			for ( var i = 1; i <= acountCount; i++) {
	    ...
	    //khglz--销管客管组第二次角色  改为可维护销售范围   by cg.jiang 201607
		...
		$.messager.confirm('Confirm', '请确认经销商资料信息,保存?', function(r) {
			...
			form.action = appUrl + "/kunnrAction!updateKunnrApplySapMeg.jspa";
			...
			if (document.getElementById("salesAcountTr_" + i)) {
				if ($("#bonus_" + i).combobox("getValue")) {
					...
					kunnrAcountList.push({
						...
						// "vtext" : $("#vtext_" + i).val(),
						"flag" : flag,
						"type2Money":$('#type2Money_'+i).val(),
						//"acountSap" : $("#text0").val(),
						"startDate" : $("#text1").datebox("getValue"),
						"endDate" : $("#text2").datebox("getValue"),
						"acountSapA" : $("#text0").val(),
						"acountSapB" : $("#text01").val(),
						"acountSapC" : $("#text02").val()
					});
				}
		//	}
		}
