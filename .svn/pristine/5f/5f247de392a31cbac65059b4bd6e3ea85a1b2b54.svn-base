			...
			.datagrid(
					{
						iconCls : 'icon-list',
						title : '数据列表',		
						url : appUrl + '/stockReport/stockReportAction!searchKunnrManagerKunnrSumList.jspa',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
...
function editeEnd(){
	...
	for(var i=0;i<rows.length;i++){
		if(rows[i].lastEstimateStock==""){
			$.messager.alert('Tips', '请维护预估上月底库存！', 'warning');
			$('#datagrid').datagrid('rejectChanges');
			return;
		}else if(rows[i].stockSafety=="" || rows[i].stockSafetyNext==""){
			$.messager.alert('Tips', '请维护安全库存！', 'warning');
			$('#datagrid').datagrid('rejectChanges');
			return;
		}
	}
...
function search() {
	...
	queryParams.stockFlag = $('#status').combobox("getValue");
