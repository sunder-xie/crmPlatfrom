	...
	$('#datagrid_2').datagrid(
					{
						iconCls : 'icon-list',
						title : '查询结果',
						url : appUrl + '/stock/instockAction!getInStockDetailListByTotalId1.jspa?type='+'I',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
...
function search_2() {
	...
	queryParams.instock_id = $("#instock_id").val();
...
function createInStock(){
	initMaintModelAtt('创建总单', '/instockAction!createStockPrepare1.jspa', 450, 350);
}

function updateInstock(id){
initMaintModelAtt('修改总单', '/instockAction!updateStockPrepare1.jspa?instock_total_id='+ id, 450, 400);
}
...
function createInStockdet(){
	...
	initMaintModelAtt('添加明细', '/instockAction!createStockdtPrepare.jspa?instock_total_id='+ instock_total_id+
