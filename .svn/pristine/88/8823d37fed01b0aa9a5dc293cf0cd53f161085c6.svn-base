			...
			.datagrid(
					{
						iconCls : 'icon-list',
						title : '数据列表',		
						url : appUrl + '/kunnrManager/kunnrManagerAction!searchKunnrManagerList.jspa',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						rownumbers : true,
...
function exportExcelCsv() {
	$.messager.confirm('Confirm', '是否确定下载模板?', function(r) {
		if (r) {
			...
			form.action = appUrl
					+ "/kunnrManagerAction!exportKunnrManagerCsv.jspa";
			form.submit();
		}
	});
						...
						buttons : [
								{
									...
									handler : function() {
										...
										if (/^.+\.(csv|CSV)$/.test(file)) {
											...
											form.action = appUrl
													+ "/kunnrManagerAction!importKunnrManagerCsv.jspa";
...
function exportForExcel() {
	...
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/kunnrManager/kunnrManagerAction!exportForExcelKunnrManager.jspa';
	form.target = "hideFrame";
	form.submit();
}
function openTime() {
...
function remove() {
	var kunnrManagerList = new Array();
	var rows = $('#datagrid').datagrid('getSelections');
	for ( var i = 0; i < rows.length; i++) {
		if(rows[i].kunnr !=null){
			kunnrManagerList.push({
				"kunnr" : rows[i].kunnr,
				"skuId" : rows[i].skuId,
				"year" : rows[i].year,
				"month" : rows[i].month
	...
	if (kunnrManagerList.length==0) {
		$.messager.alert('提示', '未选中任何权限点！', '提示');
		return;
	} else {
		$('#kunnrManagerList').val($.toJSON(kunnrManagerList));
		$.messager
				.confirm(
						'Confirm',
						'是否确认批量删除?',
						function(r) {
							if (r) {
								var form = window.document.forms[0];
								form.action = appUrl
										+ '/kunnrManager/kunnrManagerAction!deleteKunnrManager.jspa';
								form.target = "hideFrame";
								form.submit();
							}
						});
