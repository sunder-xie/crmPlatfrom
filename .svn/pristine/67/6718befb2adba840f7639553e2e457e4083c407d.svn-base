			...
			.datagrid(
					{
						...
						title : '查询结果',
						url : appUrl
								+ '/distributionInventoryAction!getDistributionInventoryList.jspa',
						loadMsg : '数据远程载入中,请等待...',
						singleSelect : false,
						pagination : true,
						nowrap : true,
...
// excel导出
function exportDistributionInventory() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionInventoryAction!exportDistributionInventory.jspa';
	form.submit();
	$.messager.alert('Tips', '正在导出,请稍候!', 'warning');
}

function closeDtPlan() {
	$("#MaintDistributionInventory").window('close');
}

function initMaintDistributionInventory(title, url, WWidth, WHeight) {
	var url = appUrl + url;
	var $win = $("#MaintDistributionInventory")
			.window(
					{
						title : title,
						width : WWidth,
...
function updateDistributionInventory(ctId) {//
	var ctId = encodeURIComponent(ctId);
	var mark = encodeURIComponent(mark);
	initMaintInventory('目标数修改', '/goalAction!updateInventoryPrepare.jspa?ctId=' + ctId
			+ '&mark' + mark, 490, 400);
}

function importDistributionInventory() {
						...
						buttons : [
								{
									...
									handler : function() {
										...
										if (/^.+\.(csv|CSV)$/.test(file)) {
											...
											var action = '';
											action = appUrl
													+ "/distributionInventory/distributionInventoryAction!importDistributionInventoryCsv.jspa";
											var form = document
...
function selectOrgTree() {
	initMaintDistributionInventory('选择组织',
			'/distributionInventoryAction!orgTreePage.jspa', 400, 460);
}
...
function importMatterModel() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionInventory/distributionInventoryAction!importDistributionInventoryModel.jspa';
	form.target = "hideFrame";
	form.submit();
}

function deleteDistributionInventory() {
	...
	$("#ids").val(ids);
	var form = window.document.forms[0];
	form.action = "distributionInventoryAction!deleteDistributionInventory.jspa";
	form.target = "hideFrame";
	form.submit();
...
function saverow(target) {
	...
	var form = window.document.forms[0];
	form.action = "distributionInventoryAction!updateDistributionInventory.jspa";
	form.target = "hideFrame";
	form.submit();
}
...
function exportMould() {
	var form = window.document.forms[0];
	form.action = appUrl
			+ '/distributionInventory/distributionInventoryAction!exportMouldCsv.jspa';
	form.target = "hideFrame";
	form.submit();
}
