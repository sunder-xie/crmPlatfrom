						...
						columns : [ [
								...
								},
								{
									...
									formatter : function(value, row, index) {
										...
										var o = '<img style="cursor:pointer" onclick="checkDistributionGoal('
												+ dgId
												+ ',3)" title="审核通过" src='
												+ imgUrl
												+ '/images/actions/icon_ok.gif align="absMiddle"></img>&nbsp&nbsp';
										var n = '<img style="cursor:pointer" onclick="checkDistributionGoal('
												+ dgId
												+ ',4)" title="驳回" src='
												+ imgUrl
												+ '/images/actions/undo.png align="absMiddle"></img>&nbsp&nbsp';
										var k = '<img title="审批" src='
												+ imgUrl
												+ '/images/actions/icon_ok_h.png align="absMiddle"></img>&nbsp&nbsp';
...
function updateDistributionGoal(dgId) {//
	var dgId = encodeURIComponent(dgId);
	initMaintDistributionGoal('目标数修改',
			'/distributionGoalAction!updateDistributionGoal.jspa?dgId=' + ctId,
			490, 400);
}
						...
						buttons : [
								{
									...
									handler : function() {
										...
										if (/^.+\.(csv|CSV)$/.test(file)) {
											...
											var action = '';
											action = appUrl
													+ "/distributionGoalAction!importDistributionGoalCsv.jspa";
											var form = document
...
function batchDistributionGoal() {
	html = '<form id="fileForm" method="post" enctype="multipart/form-data" >'
			+ '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp'
			+ '<table><tr>'
			+ '<td class="head" noWrap>选择文件上传：</td><td>'
			+ '<input type="file" name="uploadFile" id="uploadFile"  style="width:200px"/>'
			+ '</tb></tr>' + '<table></form>';
	openBatchDialog('导入目标', html);
}
var mask_;
/* 打开导入项目Excel对话框 */
function openBatchDialog(t, html) {
	if ($('#excelDialog').length < 1) {
		$(
				'<div/>',
				{
						...
						buttons : [
								{
									...
									handler : function() {
										...
										if (/^.+\.(xls|xlsx)$/.test(file)) {
											...
											var action = '';
											action = appUrl
													+ "/distributionGoalAction!batchDistributionGoalXls.jspa";
											var form = document
													.getElementById('fileForm');
...
function importMatterModel() {
	...
	form.action = appUrl
			+ '/distributionGoalAction!importDistributionGoalModel.jspa';
	form.target = "hideFrame";
	form.submit();
}
...
function exportMould() {
	var form = window.document.forms[0];
	form.action = 'distributionGoalAction!exportMouldCsv.jspa';
	form.target = "hideFrame";
	form.submit();
}
...
function saverow(target) {
	...
	var form = window.document.forms[0];
	form.action = "distributionGoalAction!updateDistributionGoal.jspa";
	form.target = "hideFrame";
	form.submit();
}
